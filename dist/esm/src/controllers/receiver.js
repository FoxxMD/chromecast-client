...
export const createReceiver = ({ client, sourceId = 'sender-0', destinationId = 'receiver-0', }) => {
    const channel = client.createChannel(sourceId, destinationId, 'urn:x-cast:com.google.cast.receiver');
    const getStatus = async () => channel.sendWithResponse({ data: { type: 'GET_STATUS' }, type: StatusResponse$ }).then(Result.map(r => r.status));
    return {
        getStatus,
        ...
        launch: (appId) => channel.sendWithResponse({ data: { type: 'LAUNCH', appId }, type: StatusResponse$ }).then(Result.map(r => r.status)),
